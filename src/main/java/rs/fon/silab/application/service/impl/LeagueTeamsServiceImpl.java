...
public class LeagueTeamsServiceImpl implements LeagueTeamsService{
    ...
    public LeagueTeamsDto save(LeagueTeamsDto ltDto) throws EntityExistsException {
        LeagueTeamsEntity.ltId id = new LeagueTeamsEntity.ltId(ltDto.getLeague().getLeagueId(), ltDto.getTeam().getTeamName());
    ...
    @Override
    public List<LeagueTeamsDto> findAllLeagues(String teamName) {
        List<LeagueTeamsEntity> teams = leagueTeamsRepository.findAllByIdTeamName(teamName);
        return teams.stream().map((entity)->{
            return leagueTeamsConverter.toDto(entity);
        }).collect(Collectors.toList());
    }
    ...
    @Override
    public void delete(Long leagueId, String teamName) {
        LeagueTeamsEntity.ltId id = new LeagueTeamsEntity.ltId(leagueId, teamName);
        leagueTeamsRepository.deleteById(id);
    }

    @Override
    public LeagueTeamsDto update(LeagueTeamsDto ltDto, Long leagueId, String teamName) throws EntityDoesntExistException{
        LeagueTeamsEntity.ltId id = new LeagueTeamsEntity.ltId(leagueId, teamName);
        Optional<LeagueTeamsEntity> entity = leagueTeamsRepository.findById(id);
        if (entity.isEmpty()){
